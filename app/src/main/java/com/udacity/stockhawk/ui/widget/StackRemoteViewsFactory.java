...
public class StackRemoteViewsFactory implements RemoteViewsService.RemoteViewsFactory {
	...
	public RemoteViews getViewAt(int position) {
		...
		if (mCursor.moveToPosition(position)) {
			final int symbolColIndex = mCursor.getColumnIndex(Contract.Quote.COLUMN_SYMBOL);
			final int priceColIndex = mCursor.getColumnIndex(Contract.Quote.COLUMN_PRICE);
			symbol = mCursor.getString(symbolColIndex);
			price = mCursor.getFloat(priceColIndex);
		}

		Timber.d("getViewAt(%s) = %s", position, symbol);
		...
		int defaultWidth = mContext.getResources().getDimensionPixelSize(R.dimen.widget_default_width);
		int largeWidth = mContext.getResources().getDimensionPixelSize(R.dimen.widget_large_width);
		...
		rv.setTextViewText(R.id.price, PrefUtils.format(price));
